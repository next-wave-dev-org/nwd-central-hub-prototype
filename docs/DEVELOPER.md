...
recovery_email          text                                          -- #99, CHECK-guarded loose email format; Security card
primary_phone           text                                          -- #99, CHECK-guarded loose phone format; Security card
```

Additive columns (`email_notifications`, and the `#98`/`#99` blocks) each have a dedicated "… Migration" section in `docs/database-schema.md`. None need a grant/RLS change — `profiles` has a table-level `UPDATE` grant to `authenticated` and the `Users can update own profile` policy covers all columns of the own row.
