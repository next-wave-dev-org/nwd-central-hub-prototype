...
| Date | Issue | Fix |
|---|---|---|
| 2026-07-06 | `app/login/admin/projects/page.tsx` queried a nonexistent `projects_table` with `.eq('status', 'Active')` (wrong case). The page always rendered "No active projects found" for admins. | Fixed the query to use `projects` / `status = 'active'`, as part of the header-unification work on #80/PR #85. |
