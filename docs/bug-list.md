...
| Date | Issue | Fix |
|---|---|---|
| 2026-07-16 | [`app/login/admin/projects/page.tsx`](../app/login/admin/projects/page.tsx) queried a nonexistent `projects_table` with `.eq('status', 'Active')` (wrong table name and case), so the page always rendered "No active projects found" for admins. | Already fixed in commit `ec398fa` ("Fix wrong table name and status case on all three Active Projects pages") — query now correctly uses `projects` / `status = 'active'`. Bug list was not updated at the time; corrected now. |
| 2026-07-16 | [`app/login/client/page.tsx`](../app/login/client/page.tsx) contained a dead, unused `ClientContent` component and `clientItems` array alongside the real exported `ClientDashboardPage`, left over from a merge. | Already resolved in commit `49363db` ("Complete PR #64 client dashboard styling that was left partially applied") — the file now contains only the single canonical `clientItems` / `ClientDashboardContent` implementation. Bug list was not updated at the time; corrected now. |
