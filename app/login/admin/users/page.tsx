...
function SortIcon({ col, sortCol, sortDir }: { col: SortCol; sortCol: SortCol | null; sortDir: SortDir }) {
  ...
  return (
    ...
    >
      <path
        d="M5 1L2 4.5h6L5 1z"
        fill={active && sortDir === 'asc' ? color : '#d1d5db'}
      />
      <path
        d="M5 11L8 7.5H2L5 11z"
        fill={active && sortDir === 'desc' ? color : '#d1d5db'}
      />
    </svg>
  )
}
...
function ManageUsersContent() {
  ...
  const pagedUsers = filteredSortedUsers.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE)
            ...
            {!loadingUsers && !usersError && filteredSortedUsers.length > 0 && (
              <div className="mb-4 flex items-center justify-between gap-4">
                <p className="text-xs text-gray-400" style={{ fontFamily: 'var(--font-geist-mono)' }}>
                  ...
                  {search && ' matching'} · page {page + 1} of {totalPages}
                </p>
